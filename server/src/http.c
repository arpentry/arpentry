#include "http.h"
#include "net.h"
#include "gen/world.h"
#include "tile_path.h"
#include "tile.h"
#include "tile_build.h"
#include "archive.h"
#include "resp_tileset.h"
#include "resp_style.h"
#include "resp_model.h"

#include <math.h>

#include <stdio.h>
#include <stdlib.h>
#include <string.h>

/* ── Server context ──────────────────────────────────────────────────── */

struct server_ctx {
    const char *tile_dir;
    const char *style_file;
    struct arpt_archive_reader *archive;
};

server_ctx *arpt_server_ctx_create(const char *tile_dir,
                                   const char *style_file,
                                   struct arpt_archive_reader *archive) {
    server_ctx *ctx = calloc(1, sizeof(*ctx));
    if (!ctx) return NULL;
    ctx->tile_dir = tile_dir;
    ctx->style_file = style_file;
    ctx->archive = archive;
    return ctx;
}

void arpt_server_ctx_free(server_ctx *ctx) {
    free(ctx);
}

/* ── Helpers ─────────────────────────────────────────────────────────── */

static const char *status_text(int status) {
    switch (status) {
    case 200:
        return "OK";
    case 400:
        return "Bad Request";
    case 404:
        return "Not Found";
    case 405:
        return "Method Not Allowed";
    case 500:
        return "Internal Server Error";
    default:
        return "Unknown";
    }
}

/* ── Pure request parsing ────────────────────────────────────────────── */

int http_parse_request(const char *data, size_t len, char *method,
                       size_t method_sz, char *uri, size_t uri_sz) {
    /* Find the end of the request line (\r\n) */
    const char *end = NULL;
    for (size_t i = 0; i + 1 < len; i++) {
        if (data[i] == '\r' && data[i + 1] == '\n') {
            end = data + i;
            break;
        }
    }
    if (!end) {
        /* Incomplete — need more data */
        return 0;
    }

    const char *line = data;
    size_t line_len = (size_t)(end - line);

    /* Parse "METHOD URI HTTP/1.x" */
    const char *sp1 = memchr(line, ' ', line_len);
    if (!sp1) return -1;

    size_t mlen = (size_t)(sp1 - line);
    if (mlen == 0 || mlen >= method_sz) return -1;
    memcpy(method, line, mlen);
    method[mlen] = '\0';

    const char *uri_start = sp1 + 1;
    size_t remaining = line_len - mlen - 1;
    const char *sp2 = memchr(uri_start, ' ', remaining);
    if (!sp2) return -1;

    size_t ulen = (size_t)(sp2 - uri_start);
    if (ulen == 0 || ulen >= uri_sz) return -1;
    memcpy(uri, uri_start, ulen);
    uri[ulen] = '\0';

    /* Return bytes consumed including \r\n */
    return (int)(end - data) + 2;
}

/* ── Response writing via net_conn ───────────────────────────────────── */

static void write_response(struct net_conn *conn, int status,
                           const char *content_type,
                           const char *content_encoding, const void *body,
                           size_t body_len) {
    char hdr[1024];
    int n = snprintf(hdr, sizeof(hdr),
                     "HTTP/1.1 %d %s\r\n"
                     "Content-Type: %s\r\n"
                     "Content-Length: %zu\r\n"
                     "Connection: close\r\n"
                     "Access-Control-Allow-Origin: *\r\n",
                     status, status_text(status), content_type, body_len);
    if (n < 0 || (size_t)n >= sizeof(hdr)) return;

    if (content_encoding) {
        int m = snprintf(hdr + n, sizeof(hdr) - (size_t)n,
                         "Content-Encoding: %s\r\n", content_encoding);
        if (m < 0 || (size_t)(n + m) >= sizeof(hdr)) return;
        n += m;
    }

    /* End of headers */
    if ((size_t)n + 2 >= sizeof(hdr)) return;
    hdr[n++] = '\r';
    hdr[n++] = '\n';

    net_conn_out_write(conn, hdr, (size_t)n);
    if (body && body_len > 0) {
        net_conn_out_write(conn, body, body_len);
    }
}

static void write_error(struct net_conn *conn, int status) {
    write_response(conn, status, "text/plain", NULL, NULL, 0);
}

/* ── Tile generation (fallback for archive misses) ───────────────────── */

/* Build a minimal tile containing just a flat terrain mesh.
   Used when an archive has no tile at the requested coordinates. */
static void *build_empty_tile(int level, int x, int y, size_t *out_size) {
    double n = (double)(1 << level);
    double west  = (double)x / n * 360.0 - 180.0;
    double east  = (double)(x + 1) / n * 360.0 - 180.0;
    double n_lat = atan(sinh(M_PI * (1.0 - 2.0 * (double)y / n))) * 180.0 / M_PI;
    double s_lat = atan(sinh(M_PI * (1.0 - 2.0 * (double)(y + 1) / n))) * 180.0 / M_PI;

    arpt_bounds bounds = { west, s_lat, east, n_lat };
    arpt_tile_builder *tb = arpt_tile_builder_create(bounds, NULL);
    if (!tb) return NULL;

    /* No features → the tile builder will inject a flat terrain mesh */
    void *data = arpt_tile_builder_finish(tb, out_size);
    arpt_tile_builder_free(tb);
    return data;
}

/* ── Respond with a Brotli-compressed resource, or 500 on failure ──── */

static void serve_resource(struct net_conn *conn, const char *content_type,
                           uint8_t *data, size_t size, bool ok) {
    if (ok) {
        write_response(conn, 200, content_type, "br", data, size);
        free(data);
    } else {
        write_error(conn, 500);
    }
}

/* ── Request dispatch ────────────────────────────────────────────────── */

static void dispatch_request(struct net_conn *conn, struct server_ctx *ctx,
                             const char *method, const char *uri) {
    /* Only accept GET */
    if (strcmp(method, "GET") != 0) {
        write_error(conn, 405);
        return;
    }

    /* Tile request: /{level}/{x}/{y}.arpt */
    int level, x, y;
    if (arpt_parse_tile_path(uri, &level, &x, &y)) {
        /* Serve from .arpa archive if available */
        if (ctx->archive) {
            size_t tile_size = 0;
            const void *tile = arpt_archive_reader_get_tile(
                ctx->archive, (uint8_t)level, (uint32_t)x, (uint32_t)y,
                &tile_size);
            if (tile) {
                write_response(conn, 200, "application/x-arpt", "br",
                               tile, tile_size);
            } else {
                /* Generate a minimal flat-terrain tile for archive misses */
                size_t empty_size = 0;
                void *empty = build_empty_tile(level, x, y, &empty_size);
                if (empty) {
                    write_response(conn, 200, "application/x-arpt", "br",
                                   empty, empty_size);
                    free(empty);
                } else {
                    write_error(conn, 500);
                }
            }
            return;
        }
        /* Fall back to procedural generation */
        uint8_t *tile_data = NULL;
        size_t tile_size = 0;
        bool ok = arpt_generate_terrain(level, x, y, &tile_data, &tile_size);
        serve_resource(conn, "application/x-arpt", tile_data, tile_size, ok);
        return;
    }

    /* Index metadata */
    if (strcmp(uri, "/index.arpi") == 0) {
        uint8_t *data = NULL;
        size_t size = 0;
        bool ok = resp_build_tileset(ctx->archive, &data, &size);
        serve_resource(conn, "application/x-arpi", data, size, ok);
        return;
    }

    /* Style */
    if (strcmp(uri, "/style.arps") == 0) {
        uint8_t *data = NULL;
        size_t size = 0;
        bool ok = resp_build_style(ctx->style_file, &data, &size);
        serve_resource(conn, "application/x-arps", data, size, ok);
        return;
    }

    /* Models */
    if (strcmp(uri, "/models.arpm") == 0) {
        uint8_t *data = NULL;
        size_t size = 0;
        bool ok = resp_build_models(&data, &size);
        serve_resource(conn, "application/x-arpm", data, size, ok);
        return;
    }

    /* Everything else: 404 */
    write_error(conn, 404);
}

/* ── Per-connection state ────────────────────────────────────────────── */

/* Maximum size of an HTTP request we'll buffer before rejecting. */
#define HTTP_MAX_REQUEST 8192

struct http_conn {
    char buf[HTTP_MAX_REQUEST];
    size_t filled;
};

http_conn *http_conn_new(void) {
    http_conn *hc = calloc(1, sizeof(*hc));
    return hc;
}

void http_conn_free(http_conn *hc) {
    free(hc);
}

void http_conn_feed(http_conn *hc, struct net_conn *conn,
                    struct server_ctx *ctx, const void *data, size_t len) {
    /* Accumulate incoming data */
    if (len > 0) {
        size_t space = sizeof(hc->buf) - hc->filled;
        size_t copy = len < space ? len : space;
        memcpy(hc->buf + hc->filled, data, copy);
        hc->filled += copy;
    }

    /* Reject oversized requests */
    if (hc->filled >= sizeof(hc->buf)) {
        write_error(conn, 400);
        net_conn_close(conn);
        return;
    }

    /* Try to parse a complete request */
    char method[8];
    char uri[2048];
    int consumed = http_parse_request(hc->buf, hc->filled, method,
                                      sizeof(method), uri, sizeof(uri));
    if (consumed == 0) {
        /* Incomplete, wait for more data */
        return;
    }
    if (consumed < 0) {
        write_error(conn, 400);
        net_conn_close(conn);
        return;
    }

    /* One request per connection (Connection: close). */
    dispatch_request(conn, ctx, method, uri);
    net_conn_close(conn);
}
