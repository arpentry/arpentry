...
/* Assign a geometry to tiles at the given zoom level.  For polygons,
   orig is the unsimplified geometry used for interior-tile containment
   tests (may equal geom if no simplification was applied). */
...
/* Process a geometry across a zoom range: simplify incrementally from
   finest to coarsest, skip sub-pixel features, then assign to tiles
   via the callback. */
