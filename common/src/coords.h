...
/* Dequantize a uint16 coordinate back to normalized tile space [-0.5, ~1.5]. */
static inline double arpt_dequantize(uint16_t q) {
    return ((double)q - 16384.0) / 32768.0;
}

/* Quantize a normalized coordinate to uint16, clamped to [0, 65535]. */
...
/* Geodetic bounds (degrees) for a tile at the given level, x, y. */
...
/* Deterministic geometric error: root_error / 2^level. */
