...
| `order.walk_level` | I3 | Stroke height minus the at-grade band top, over every vertex of every pedestrian-class *stroke* emitted at level ≥ 1 that shares a plan position with an at-grade surface. The stroke population `order.deck_above_carriageway` cannot see: an elevated draped span that declines to stamp a solid (`synth::draped` → `Stamped::Degrade`) falls back to a plain ground drape, leaving a level-1 line and no mesh — drawn in the underpass its own ordinal says it flies over. Its gorge caveat is inherited from the deck metric: sharing a plan position is not crossing. |
...
| `seam.band_deck_bare` | I2 | Drawn ground between the at-grade *surface band* and the bridge deck that continues it, marched out along the band's own silhouette. The surface half of `seam.abutment_*`, and the only abutment instrument at the zooms where the road surface exists: from `ROAD_SURFACE_MIN_ZOOM` the tiler deletes the stroke of every class the union paves — carriageway and rail formation alike (`pipeline::paves_via_union`) — so the stroke check has no detail-rung population at all. Neither modality hides the joint any more: the rail track ribbon that used to be drawn over both the ballast band and the deck is deleted with the carriageway's stroke, so a ballast band must meet its deck as nakedly as asphalt does. Pairing is directional on both footprints, judged by extents of the same point-in-footprint query the march trusts (a swept solid has no boundary silhouette to read an axis from): the band must be left through an *end* — extending behind the march at least half as far as across it — and the deck *entered*, not struck broadside, so a twin alignment's bridge across the median is a neighbour, never a handover; the march runs along the band's own axis (best end-gate margin over the anchor normal and ±45°, the gate itself judged on the normal so rotations cannot widen the population), looks one rim laterally at every step (an anchor centimetres beside a complete deck otherwise grazes its face for the span's length), trusts a deck at matching height within 2 m without the entry ratio (an oblique joint's acute corner reads broadside one step out), and skips entirely within 6 m of the tile-proper border, where the clip splits a lengthwise-crossed joint's evidence between two tiles. Each of those five rules retired a measured artifact class that was, at its worst, the whole of the metric's tail. |
...
| `paint.stroke_over_band` | I1 | Share of at-grade pedestrian-stroke vertices (footway, path, steps, cycleway, pedestrian, track) standing on a drawn `walk_*`/`path_*` band, on tiles that draw one. A way the walkway model banded has its cartographic stroke deleted from `WALK_SURFACE_MIN_ZOOM` — the band *is* the way there — so a stroke over a band is a line painted on its own surface. The mode it keeps dead: the deletion test read two properties phase 1 stamped on the feature, and `profile::profile` builds tile properties from a fixed whitelist of *source* attributes, so the stamps never reached the tile and the test could never fire. Every banded way drew twice — **351 km of stroke at z16 on the Montreux extract, 82 % of it over its own band**, measured 78.9 % here before and 3.6 % after. The floor is not zero: an unbanded way's last vertices stand on the pavement they run into — 46 of 494 vertices on the Montreux zone, over 26 of its 178 still-stroked features, **37 of the 46 within two vertices of a part end**, hit runs a median of 1 vertex long. The residue scales with joins, not with length, so read the count; the population shrinks as the model bands more, which inflates the share a fixed residue represents. A reading that grows in the *middle* of features is the defect coming back. |
...
| `paint.marking_on_crossing` | I4 | Share of at-grade `marking` vertices standing inside a zebra ladder — within 0.5 m of a `crossing` bar at their own height — on tiles that draw a ladder at all. Two painted systems contradicting each other on one asphalt: the dash filter dropped a dash only where its midpoint lay on a junction *area* (`synth::markings::midpoint_paved`), and a crossing is not an area, so a centre line rode straight through the ladder — first measured as a dash 0.28 m from the bars at a Territet junction mouth (6.90896, 46.44008), 2.04 % of that tile's marking vertices. The bake now drops any dash reaching inside the ladder's footprint plus a metre of stop margin (`synth::markings::ChordIndex`); the margin is a metre and not the bar clearance because dashes are cut on the model-space line and drawn snapped to the smoothed sweep, which displaces them decimetres near a curve. The 0.5 m gate is half the ladder's bar period, so the measure cannot slip between bars, and it sits inside the stop margin, so anything it reads is the chord plumbing having failed rather than the threshold arguing with the rule. The height reach (1.5 m) keeps a deck's centre line over an underpass crossing out of the population. |
...
| `solve.residual_*` | I2/I3 | One row per constraint family of the relaxation (`grade`, `bore_ceiling`, `deviation`, `clearance`, `undercut`, `rigidity`, `monotone`, `contact`): over every solved variable of every stratum's graph, the distance one further application of the family's **own pass** would move it, measured after the closing settle with the heights restored between families. A point is feasible for a projection exactly when the projection fixes it, so the pass is the instrument and no second construction of the constraint math can drift from the one the solver enforces. This is the 2026-08-22 review's finding 3 answered: the pass order in `relax::solve` is load-bearing and earned, and before these rows the only evidence it worked on a given scene was the absence of drawn artifacts downstream — reordering or adding a pass is now a measured change, which is the de-risk the §3.3 two-pass refactor waits on. First measurement (Montreux, 390,634 vars): `bore_ceiling`, `undercut` and `contact` hold *exactly* — the settle's "last word" comments, verified — and `grade` is the sacrificed family (0.67 %, worst 32.9 m of distance-to-feasible: the settle converges grade first and every later projection may spend it). Threshold 1 mm — ten times the solver's own `TOL_M`, under which a move is the limit cycle the settle exists to absorb. |
| `partition.divergence` | I2 | Metres of centerline per structure-bearing corridor on which `solve::partition` — the fold's own predicates (`annex_spans`, `reconcile_spans`) as one pure function, plus the bridge half clamped to the derived deck runs — names a different kind than the spans the fold wrote, split by family in the detail. The two-pass refactor's first instrument (`data/plans/pure-partition-2026-08-28.md` §5 step 1). First measurement 2026-08-30 (Montreux zone): 594 corridors, median 0.43 m, 100 % of it `bridge→grade` — the withdrawn slice's family and nothing else — with a tail of DEM-blind viaducts whose deck the DEM already carries (A9, 757 m). |
...
| `street.crossing_extent` | I4 | Every registered crossing chord with an end inside the bbox, marched at 0.25 m and scored as the total length lying further than its own drawn half-width from every corridor **the mapped crosswalk polyline actually crosses** — properly intersects, at a believable incidence (≥ 20°), at the crossing's own ground level (± 3 m of the DEM at the mapped midpoint), within 25 m of arc of the crossing station. A crossing crosses one street at one station and at one level; the registration (`synth::walkway::crossings`) now makes all four distinctions with the same shared predicates (`crossed_hosts`, `host_level_ok`, `incidence_ok`, `CROSSING_ARC_WINDOW_M`), so check and generator share one derivation and this metric watches the plumbing rather than arguing with the rule. The check derives hosts from the *extended* line exactly as the registration does — refusing the extension scored the second roadway of a divided crossing as 4.7 m of a 5 m chord bare, on a drawing that is right. The modes each gate killed, measured 2026-09-01 at Territet: a station forecourt's service roads annexed by plan-only marching; a 20 m ladder climbing 6.4 m over an underpass portal; a terrace crosswalk registered against the avenue 13 m below — worst 9.4 m → 0 on the roundabout bbox once check and generator shared the whole derivation. Crossings whose polyline crosses no drivable centerline are out of the population — nothing owes them a width — and their count is reported separately under `ARPT_DEBUG_STRIP`. |
| `street.crossing_skew` | I4 | One sample per (crossed centerline, chord) pair: every place a crossing's mapped polyline crosses a drivable centerline — under the same level and incidence gates the registration uses — scored as the angle in degrees between the crossing's *nearest* registered chord and square-across that centerline. Since the R7 finish the bars run along the crossed street's own tangent whatever the chord does (`synth::markings::crossing_bars` takes the registration's host tangent per chord, shear bounded at 45° so a chord mapped 73° off square draws leaning stripes rather than a smear; `ARPT_NO_BAR_TRAFFIC` restores chord-square bars) — an oblique chord draws a *sheared* ladder with stripes still longitudinal to traffic, so this is a registration metric now, not a drawn symptom. Pairing is by the crossing's own chords only, and per crossed street: pairing by proximity alone scored corner crossings against the *other* street of their junction, which reads ~90° however square both ladders are; a plan-only host set did the same with the avenue a terrace below, and a hairpin's far arm grazed at ~2° of incidence read 88.8° until the gates landed. What stands above the gate now is faithful obliquity's tail — 38.9° where a crossing continues diagonally over a side roadway's curving mouth — whose drawn symptom the bars-parallel-to-traffic finish (R7, landed 2026-09-03) drained; the number itself still watches the chord derivation. Obliquity can be real (a refuge island, a bent kerb — the Territet junction's visually diagonal ladder measures 12.2° against the exact centerline tangent and is faithful), so the 20° gate is loose. `ARPT_DEBUG_STRIP` prints every sample. |
...
- **The band does not draw its own edge.** The trap that decides whether
  ...
  paved region — the outer `PAVE_RIM_M` (0.35 m) is a separate `road_rim`
  ...
  but counts interior and rim alike as surface, and reports the distance
