...
| `street.crossing_skew` | I4 | One sample per (crossed centerline, chord) pair: every place a crossing's mapped polyline crosses a drivable centerline — under the same level and incidence gates the registration uses — scored as the angle in degrees between the crossing's *nearest* registered chord and square-across that centerline. The bars are drawn perpendicular to the chord (`synth::markings::crossing_bars`), so this is exactly how far every bar in the ladder lies from longitudinal to traffic — the drawn symptom is a zebra rotated against its own street. Pairing is by the crossing's own chords only, and per crossed street: pairing by proximity alone scored corner crossings against the *other* street of their junction, which reads ~90° however square both ladders are; a plan-only host set did the same with the avenue a terrace below, and a hairpin's far arm grazed at ~2° of incidence read 88.8° until the gates landed. What stands above the gate now is faithful obliquity's tail — 38.9° where a crossing continues diagonally over a side roadway's curving mouth — which is the population a bars-parallel-to-traffic finish (R7) would drain. Obliquity can be real (a refuge island, a bent kerb — the Territet junction's visually diagonal ladder measures 12.2° against the exact centerline tangent and is faithful), so the 20° gate is loose. `ARPT_DEBUG_STRIP` prints every sample. |
